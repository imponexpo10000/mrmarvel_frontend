...
export const getAdminProduct = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${backend_api}/api/v1/admin/products`);
...
export const createProduct = (productData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      `${backend_api}/api/v1/admin/product/new`,
      productData,
      config
    );
...
export const getAllReviews = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${backend_api}/api/v1/reviews?id=${id}`);
...
export const deleteReviews = (reviewId, productId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(
      `${backend_api}/api/v1/reviews?id=${reviewId}&productId=${productId}`
    );
